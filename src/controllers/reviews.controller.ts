...
import TrendingService from "../helpers/modelHelpers/businessModelHelpers/trending.service";
...
export default class ReviewsController {
  ...
  public createReview = async (
    ...
    try {
      ...
      this.reviewsService.updateItemRatingByItemId(
        req.body.idOfItem.toString()
      );

      await this.trendingService.updateVendorRating(req.body.idOfItem);

      await this.trendingService.setBestReviews();
    } catch (err) {
      next(err);
    }
  };
  ...
  public getReviews = async (
    ...
    try {
      const reviews: any = await this.reviewsService.findReviewsByItemId(
        req.params.id
      ); // Use the helper function to find the reviews with itemId
      console.log(reviews);
